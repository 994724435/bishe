...
class IndexAction extends CommonAction{
	...
	public function main(){
		...
		$admin   =M('admin');
		if($_POST['add']){
			if($_POST['end1']<$_POST['start1']+$_POST['add']){
				echo "<script type='text/javascript'>alert('对不起，加价后大于一口价！');location.href='".__ROOT__."/index.php/Index/main';</script>";
			}else{
				$array['start'] = $_POST['start1']+$_POST['add'];
				$res  = $product->where(array('id'=>$_POST['id']))->select();
				if($res[0]['buyer']==$_SESSION['name']){
				$array['start2'] = $_POST['start1']+$_POST['add'];
				$array['start'] = $_POST['start1']+$_POST['add'];
				$array['buyer'] = $_SESSION['name'];				
				$result =$product->where(array('id'=>$_POST['id']))->save($array);
				}else{
				$array['start1'] = $_POST['start1']+$_POST['add'];
				$array['buyer1'] = $_SESSION['name'];
				$array['start'] = $_POST['start1']+$_POST['add'];				
				$result =$product->where(array('id'=>$_POST['id']))->save($array);					
				}

				if($result){
				echo "<script type='text/javascript'>alert('竞拍成功！');location.href='".__ROOT__."/index.php/Index/main';</script>";	
				}
			}
		}
		if($_POST['name']){
...
//		$mine_jing = $product->where(array('buyer'=>$_SESSION['name'],'issale'=>0))->select(); 
		$mine_jing = $product->where(array('buyer'=>$_SESSION['name'],'issale'=>0))->select(); 
		$mine_jing1 = $product->where(array('buyer1'=>$_SESSION['name'],'issale'=>0))->select(); 
		$mine_buyed=$product->where(array('buyer'=>$_SESSION['name'],'issale'=>1))->select();
		
		
//		已出售的
        $doneSale = $product->where(array('issale'=>1,'admin'=>$_SESSION['name']))->select();
        $this->assign('doneSale',$doneSale);
		$this->assign('product',$onSale);
		$this->assign('mine',$mine_buyed);
		$this->assign('mine_jing',$mine_jing);
		$this->assign('mine_jing1',$mine_jing1);
		$this->display();
	}
    //用户购买
    public function buy(){
